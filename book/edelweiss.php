...
if (isset($_GET['date'])) {
    $date = $_GET['date'];
    // UBAH BOOKINGS SESUAI DATABASE
    $stmt = $mysqli->prepare("select * from edelweiss where date = ?");
...
if(isset($_POST['submit'])){
    ...
    $timeslot = $_POST['timeslot'];
                           // UBAH BOOKINGS SESUAI DATABASE
    $stmt = $mysqli->prepare ("select * from edelweiss where date = ? AND timeslot =  ?");
    ...
    if($stmt->execute()){
        ...
        if($result->num_rows>0){
          $msg = "<div class='alert alert-Danger'>Already Booked</div>";
            ...
            // UBAH BOOKINGS SESUAI DATABASE
          $stmt = $mysqli->prepare("INSERT INTO edelweiss (name, timeslot, email, date) VALUES (?,?,?,?)");
